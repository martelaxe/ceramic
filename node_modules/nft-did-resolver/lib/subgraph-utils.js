...
export const fetchQueryData = async (queryUrl, query) => {
    const fetchOpts = {
        method: 'POST',
        headers: {
 ...
 */
export const blockAtTime = async (timestamp, blockQueryUrl) => {
    const query = {
        blocks: {
            __args: {
                first: 1,
    ...
    const queryData = (await fetchQueryData(blockQueryUrl, query));
    if (!queryData?.blocks) {
 ...
 */
export const isWithinLastBlock = (timestamp, skew) => {
    return Date.now() - timestamp <= skew;
};
export const erc721OwnerOf = async (asset, blockNum, queryUrl) => {
    ...
    const queryData = (await fetchQueryData(queryUrl, query));
    if (!queryData?.tokens) {
        throw new Error('Missing data from subgraph query');
    }
    else if (queryData.tokens.length === 0) {
        throw new Error(`No owner found for ERC721 NFT ID: ${asset.tokenId} for contract: ${asset.assetName.reference}`);
    }
    return queryData.tokens[0].owner.id;
};
export const erc1155OwnersOf = async (asset, blockNum, queryUrl) => {
    const tokenId = `0x${new BigNumber(asset.tokenId).toString(16)}`;
    const query = {
        tokens: {
            __args: {
    ...
    if (!queryData?.tokens[0]) {
        throw new Error(`No tokens with ERC1155 NFT ID: ${asset.tokenId} found for contract: ${asset.assetName.reference}`);
    }
    else if (!queryData.tokens[0].balances || queryData.tokens[0].balances.length === 0) {
        throw new Error(`No owner found for ERC1155 NFT ID: ${asset.tokenId} for contract: ${asset.assetName.reference}`);
    }
    return queryData.tokens[0].balances.slice().map((bal) => bal.account.id);
};
//# sourceMappingURL=subgraph-utils.js.map